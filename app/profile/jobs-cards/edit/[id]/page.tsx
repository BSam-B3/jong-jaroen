...
export default function EditJobsCardPage() {
  const router = useRouter();
  const params = useParams<{ id: string }>(); // 🌟 ดึง ID ของงานจาก URL
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      // 🌟 ดึงข้อมูลงานเดิมมาแสดง
      if (params?.id) {
        const { data: serviceData, error: serviceError } = await supabase
          .from('services')
          .select('*')
          .eq('id', params.id)
          .eq('provider_id', session.user.id) // ป้องกันคนอื่นมาแก้
          .single();

        if (serviceData) {
          setTitle(serviceData.title || '');
          setCategory(serviceData.category || CATEGORIES[0]);
          setDescription(serviceData.description || '');
          
          // จัดการรูปภาพ (ถ้ามีไม่ถึง 5 รูป ให้เติมช่องว่างให้ครบ)
          const loadedImages = serviceData.images || [];
          const fullImages = Array(5).fill('').map((_, i) => loadedImages[i] || '');
          setImages(fullImages);

          // จัดการแพ็กเกจ
          if (serviceData.packages && serviceData.packages.length === 3) {
            setPackages(serviceData.packages);
          }
        } else if (serviceError) {
          alert('ไม่พบข้อมูลบริการ หรือคุณไม่มีสิทธิ์แก้ไขงานนี้ค่ะ');
          router.push('/profile/jobs-cards');
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq('id', params.id)
        .eq('provider_id', user.id); // ยืนยันสิทธิ์อีกครั้ง

      if (error) throw error;
      alert('💾 บันทึกการแก้ไข Jobs-Card เรียบร้อยแล้วค่ะ!');
      router.push('/profile/jobs-cards'); // กลับไปหน้ากระเป๋า
    } catch (err: any) {
  ...
  if (fetchingData) {
    return (
      <div className="min-h-screen bg-[#F0F4F0] flex justify-center items-center font-sans">
        ...
        <header className="sticky top-0 z-[100] bg-white/95 backdrop-blur-md border-b border-gray-200 px-4 md:px-8 py-3 md:py-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <div className="w-8 h-8 md:w-10 md:h-10 bg-[#00C300] rounded-lg p-1.5 shadow-inner flex items-center justify-center">
                <span className="text-white text-lg md:text-xl">✏️</span>
              </div>
              <div className="hidden sm:block">
                <h1 className="text-[#00C300] font-black text-base md:text-lg leading-none tracking-tight">แก้ไขข้อมูล</h1>
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Edit Jobs-Card</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex flex-col items-end mr-2">
              <span className="text-[10px] font-bold text-gray-400">ตรวจสอบการแก้ไขให้ครบถ้วน</span>
            ...
            >
              {loading ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></div> : '💾 บันทึกการแก้ไข'}
            </button>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} className="p-4 md:p-8 space-y-12">
          ...
          <section id="overview" className="scroll-mt-36">
            ...
            <div className="space-y-5">
              <div>
                <label className="text-xs font-bold text-gray-500">หัวข้อ Jobs-Card ของคุณ <span className="text-red-500 ml-1">**</span></label>
                <input 
                  type="text" 
        ...
        <div className="fixed bottom-[80px] md:bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t-4 border-[#00C300] p-4 md:px-8 z-[100] flex justify-center shadow-[0_-10px_20px_rgba(0,195,0,0.08)]">
          <div className="w-full max-w-4xl flex items-center justify-between gap-4">
            <div className="hidden md:block">
              <p className="text-xs font-bold text-gray-500">ตรวจสอบข้อมูลให้ครบถ้วนก่อนบันทึกการแก้ไข</p>
              <p className="text-[10px] text-gray-400">สร้าง Jobs-Card ฟรี ไม่มีค่าใช้จ่าย (จงเจริญ)</p>
            </div>
            <button 
              onClick={handleSubmit}
              disabled={loading || !isFormValid}
              className={`w-full md:w-auto px-10 py-4 text-white rounded-2xl font-black text-sm md:text-base active:scale-95 transition-all flex justify-center items-center gap-2 shadow-xl ${
                isFormValid 
                  ? 'bg-gradient-to-r from-[#00C300] to-[#00A300] hover:shadow-green-200' 
                  : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
              }`}
            >
              {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : '💾 บันทึกการแก้ไข'}
            </button>
          </div>
        </div>
