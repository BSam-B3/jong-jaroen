...
function ChatContent() {
  ...
  // 🌟 States สำหรับการจัดการงาน (Job Actions)
  ...
  // 2. ดึงข้อความแชท และ Subscribe Realtime เมื่อเลือกห้อง
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#F4F6F8] font-sans flex flex-col md:flex-row max-w-6xl mx-auto md:p-6 h-screen">
      ...
      {activeChat ? (
        <div className={`flex-1 bg-[#F8FAFC] flex flex-col md:rounded-r-[2rem] shadow-sm relative h-screen md:h-full`}>
          ...
          <header className="bg-white p-4 border-b border-gray-100 flex flex-col gap-3 shadow-sm z-10 md:rounded-tr-[2rem]">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  <span className="text-[9px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">Ref: #{currentJob?.id.substring(0, 6).toUpperCase()}</span>
                  <span className="text-[9px] font-black text-[#EE4D2D] uppercase tracking-widest">สถานะ: {currentJob?.status}</span>
                </div>
              </div>
            </div>

            {/* 🌟 Action Panel (ปุ่มควบคุมงาน) */}
            <div className="pt-2 border-t border-gray-50">
              ...
              {isEmployer && currentJob?.status === 'open' && (
                <Link href={`/checkout/${currentJob.id}`} className="block w-full bg-[#00C300] text-white py-2 rounded-xl text-xs font-black shadow-md hover:bg-[#00A300] transition-colors text-center">
                  ✅ ยืนยันจ้างงาน & ชำระเงิน (Escrow)
                </Link>
              )}

              {isWorker && currentJob?.status === 'in_progress' && (
                <button onClick={() => fileInputRef.current?.click()} disabled={isActionLoading} className="w-full bg-[#EE4D2D] text-white py-2 rounded-xl font-black text-xs shadow-md active:scale-95 transition-all">
                  {isActionLoading ? 'กำลังอัปโหลด...' : '📸 ถ่ายรูปผลงาน & ส่งมอบ (เก็บตังค์!)'}
                </button>
              )}

              {isEmployer && currentJob?.status === 'delivered' && (
                <button onClick={handleReleaseFunds} disabled={isActionLoading} className="w-full bg-emerald-500 text-white py-2 rounded-xl font-black text-xs shadow-md active:scale-95 transition-all">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
            <div className="text-center mb-4">
              <span className="text-[10px] font-bold text-gray-500 bg-white border border-gray-200 px-4 py-1.5 rounded-full shadow-sm">
                🛡️ การสนทนานี้ปลอดภัยด้วยระบบจงเจริญ
              </span>
            </div>

            {/* 📸 แสดงรูปผลงานที่ช่างอัปโหลด (แทรกกลางแชทถ้ามี) */}
          ...
          {/* ⌨️ Input & Quick Reply Area */}
          <div className="bg-white border-t border-gray-100 p-3 md:p-4 md:rounded-br-[2rem]">
