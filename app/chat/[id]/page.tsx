...
function ChatHubContent() {
  ...
  useEffect(() => {
    const initChat = async () => {
      ...
      // ดึงรายชื่อห้องแชท (ดึง job_type มาด้วยเพื่อแยกไอคอน)
  ...
  const isJobOnsite = currentJob?.job_type === 'onsite';
      ...
      {activeChat ? (
        <div className={`flex-1 bg-[#F8FAFC] flex flex-col md:rounded-r-[2rem] shadow-sm relative h-screen md:h-full`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                  <span className="text-[9px] font-black text-white bg-gray-800 px-1.5 py-0.5 rounded shadow-sm">
                    {isJobOnsite ? '📍 ONSITE' : '💻 ONLINE'}
                  </span>
          ...
          <div className="p-3 md:p-4 bg-white border-t border-gray-100 md:rounded-br-[2rem]">
            {/* 🌟 Dynamic Quick Replies แยกตามประเภทงาน */}
            <div className="flex gap-2 overflow-x-auto no-scrollbar mb-3 pb-1 px-1">
               {isWorker ? (
                 isJobOnsite ? (
                   <>
                    <button onClick={() => handleSendMessage(undefined, "📍 กำลังเดินทางไปครับ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">📍 กำลังไป</button>
                    <button onClick={() => handleSendMessage(undefined, "📦 ถึงที่หมายแล้วครับ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">📦 ถึงแล้ว</button>
                   </>
                 ) : (
                   <>
                    <button onClick={() => handleSendMessage(undefined, "💻 พร้อมเริ่มงานแล้วครับ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">💻 เริ่มงาน</button>
                    <button onClick={() => handleSendMessage(undefined, "📥 ส่งดราฟต์ให้ตรวจสอบครับ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">📥 ส่งดราฟต์</button>
                   </>
                 )
               ) : (
                 isJobOnsite ? (
                   <>
                    <button onClick={() => handleSendMessage(undefined, "📍 ตอนนี้อยู่ไหนแล้วคะ?")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">📍 อยู่ไหนแล้ว?</button>
                    <button onClick={() => handleSendMessage(undefined, "✅ รออยู่ที่จุดนัดหมายค่ะ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">✅ รออยู่ค่ะ</button>
                   </>
                 ) : (
                   <>
                    <button onClick={() => handleSendMessage(undefined, "✏️ ขอปรับแก้ตรงนี้หน่อยค่ะ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">✏️ ขอปรับแก้</button>
                    <button onClick={() => handleSendMessage(undefined, "✅ งานเรียบร้อยดีมากค่ะ")} className="shrink-0 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-full text-[10px] font-bold text-gray-600 hover:bg-orange-50 transition-colors">✅ งานโอเคค่ะ</button>
                   </>
                 )
               )}
            </div>
            ...
            <form onSubmit={handleSendMessage} className="flex gap-2">
              ...
              <button 
                ...
                className="w-12 h-12 bg-[#EE4D2D] text-white rounded-full flex items-center justify-center shadow-md active:scale-95 disabled:opacity-50 transition-all shrink-0"
              >
