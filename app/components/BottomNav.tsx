...
export default function BottomNav() {
  ...
  useEffect(() => {
    // 1. ตรวจสอบเบื้องต้นว่ามีแจ้งเตือนที่ยังไม่ได้อ่านไหม
    const checkInitialNotes = async () => {
    ...
    // 2. ดักฟังแจ้งเตือนใหม่แบบ Real-time
  ...
  // เมื่อกดเข้าหน้าแจ้งเตือนแล้ว ให้เคลียร์จุดแดงออก
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-0 w-full sm:max-w-2xl md:max-w-3xl left-1/2 -translate-x-1/2 z-50">
      <nav className="bg-white/95 backdrop-blur-md border-t border-gray-100 px-2 py-2 flex items-center shadow-[0_-4px_25px_rgba(0,0,0,0.06)] rounded-t-[2.5rem]">
        ...
        {/* ส่งค่า hasNewNote เข้าไปที่ hasBadge เพื่อคุมการโชว์จุดแดง */}
        <NavItem 
          icon="🔔" 
          label="แจ้งเตือน" 
          href="/notifications" 
