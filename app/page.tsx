...
export default function HomePage() {
  ...
  return (
    /* 📱 พื้นหลังด้านนอก (Desktop/Tablet) */
    <div className="min-h-screen bg-gray-100 flex justify-center">
      
      {/* 🧩 ตัวแอป (Responsive Container) - ขยายตามจอแต่คุมความกว้างไว้ไม่ให้ยืดเกินไป */}
      <div className="w-full sm:max-w-2xl md:max-w-3xl bg-[#F4F6F8] min-h-screen pb-28 shadow-xl relative flex flex-col">
        ...
        <div className="bg-[#F05D40] rounded-b-[2.5rem] p-6 pt-12 shadow-sm relative">
          <div className="flex justify-between items-start mb-6 px-2">
            ...
            {/* แสดงชื่อและอักษรย่อ User แทนตัว N เดิม */}
            <div className="flex items-center gap-2 bg-white/10 p-1.5 pr-4 rounded-full border border-white/20 backdrop-blur-md">
          ...
          {/* 🔍 Search Bar */}
          <div className="bg-white rounded-2xl p-3.5 flex items-center shadow-lg transform translate-y-2 mx-2">
        ...
        <div className="p-5 space-y-6 mt-4">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-6 px-1">
              ...
              <button className="text-[#F05D40] text-xs font-bold bg-orange-50 px-4 py-1.5 rounded-full">ดูทั้งหมด</button>
            </div>
            <div className="grid grid-cols-4 gap-y-8 gap-x-2">
              {[
                ...
                <div key={index} className="flex flex-col items-center gap-2 active:scale-90 transition-transform cursor-pointer">
                  <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center text-2xl border border-gray-100 shadow-sm">{item.icon}</div>
        ...
        <div className="fixed bottom-0 w-full sm:max-w-2xl md:max-w-3xl bg-white/90 backdrop-blur-md border-t border-gray-100 px-8 py-4 flex justify-between items-center shadow-[0_-4px_25px_rgba(0,0,0,0.06)] rounded-t-[2.5rem] z-50">
          ...
          <NavItem icon="🛠️" label="บริการ" active={activeTab === 'services'} onClick={() => {setActiveTab('services'); router.push('/hire')}} />
          ...
          <NavItem icon="👤" label="ฉัน" active={activeTab === 'profile'} onClick={() => {setActiveTab('profile'); router.push('/profile')}} />
        </div>
...
function MenuButton({ icon, title, desc, color, badge, onClick }: any) {
  const colorClass = color === 'blue' ? 'bg-blue-50 border-blue-50' : 'bg-orange-50 border-orange-50';
  return (
    <div onClick={onClick} className={`bg-white p-6 rounded-[2rem] shadow-sm border ${colorClass} flex flex-col items-center justify-center text-center active:scale-95 transition-all cursor-pointer relative overflow-hidden h-full`}>
      {badge && <div className="absolute top-0 right-0 bg-[#F05D40] text-white text-[9px] font-black px-4 py-1 rounded-bl-2xl shadow-sm">{badge}</div>}
      <div className={`w-16 h-16 ${colorClass} rounded-3xl flex items-center justify-center text-4xl mb-4 shadow-inner`}>{icon}</div>
...
function NavItem({ icon, label, active, onClick }: any) {
  return (
    <div onClick={onClick} className={`flex flex-col items-center gap-1.5 cursor-pointer transition-all ${active ? 'scale-110' : 'opacity-40'}`}>
